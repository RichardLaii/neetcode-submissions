...
class Solution {
public:
    unordered_map<int, int> mp; // map inorder
    ...
    int inStart, int inEnd){
        ...
        int mid = mp[rootVal]; // find the root in inorder
        int leftSize = mid - inStart;

        root->left = build(preorder, preStart + 1 /*first one is root*/, preStart + leftSize,
